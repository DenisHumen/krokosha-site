...
main() {
  ...
  systemctl disable --now krokosha-sync.timer krokosha-rebuild.path krokosha-mailbox.path krokosha-api.service \
    krokosha-backup.timer krokosha-backup-now.path krokosha-certwatch.timer krokosha-geoipupdate.timer krokosha-dbip.timer \
    krokosha-netmap.timer krokosha-fail2ban.timer 2>/dev/null || true
  systemctl stop krokosha-sync.service krokosha-backup.service krokosha-certwatch.service krokosha-geoipupdate.service \
    krokosha-dbip.service krokosha-netmap.service krokosha-fail2ban.service 2>/dev/null || true
  rm -rf /etc/systemd/system/krokosha-mailbox.service /etc/systemd/system/krokosha-mailbox.path \
    /etc/systemd/system/krokosha-sync.service /etc/systemd/system/krokosha-sync.service.d /etc/systemd/system/krokosha-sync.timer \
    /etc/systemd/system/krokosha-backup.service /etc/systemd/system/krokosha-backup.timer /etc/systemd/system/krokosha-backup-now.path \
    /etc/systemd/system/krokosha-fail2ban.service /etc/systemd/system/krokosha-fail2ban.timer \
