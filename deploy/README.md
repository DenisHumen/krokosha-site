...
- **Ящики из админки** — раздел «Почта»: новый ящик для сотрудника (пароль генерируется и показывается один раз или задаётся вручную), новый пароль, удаление. У веб-сервиса нет прав на почтовый сервер: он кладёт запрос — действие, адрес и хэш пароля — в `/var/lib/krokosha/requests/mail`, а `krokosha-mailbox.path` запускает `krokosha-mailbox apply` от root, который проверяет запрос заново и применяет. Список адресов и журнал операций для админки — в `/var/lib/krokosha/mail` (каталог root, сервис только читает). Служебный `leads@` из админки не меняется и не удаляется.
