...
| GeoIP | `/var/lib/GeoIP/GeoLite2-City.mmdb` — база стран и городов, `geoipupdate` обновляет её дважды в неделю (`krokosha-geoipupdate.timer`); ключ MaxMind — в `/etc/GeoIP.conf` (`600 root`) и в `/etc/krokosha/env` |
...
Страна и город на экранах статистики и в карточках заявок определяются по локальной базе MaxMind GeoLite2 (сеть не спрашивается, адрес не хранится). База бесплатная, но нужен аккаунт: [signup](https://www.maxmind.com/en/geolite2/signup) → в аккаунте «Manage License Keys» → ключ в файл, затем:

```bash
sudo /opt/krokosha/repo/deploy/install.sh --from-env --maxmind-account 123456 --maxmind-key-file /root/maxmind.key
```

Установщик ставит `geoipupdate`, пишет `/etc/GeoIP.conf` (только root), скачивает `GeoLite2-City.mmdb` и включает `krokosha-geoipupdate.timer` (среда и суббота — MaxMind публикует базу по вторникам и пятницам). API замечает новый файл сам, перезапуск не нужен. Без ключа всё работает, а страны остаются «не определены»; экран «Статус системы» показывает, какая база загружена и когда собрана, и предупреждает, если она старше шести недель.
...
│                       krokosha-backup, krokosha-certwatch, krokosha-geoipupdate (.service + .timer)
...
└── ci/                 test-install.sh — интеграционный тест установщика; притворные Telegram и IndexNow,
