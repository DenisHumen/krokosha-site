...
├── handoff.md                 выкатка и проверка текущей ветки: CI, update.sh, чек-лист, Search Console
...
└── telegram.md                как устроено общение через Telegram: для владельца и для разработчика
```
