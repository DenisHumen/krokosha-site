...
**Версия 1.13** (2026-09-24). Основа — часть C брифа ([brief/MASTER_PROMPT.md](brief/MASTER_PROMPT.md)). Всё, что добавлено сверх брифа, помечено **[v1.1]** / **[v1.2]** / **[v1.3]** / **[v1.4]** / **[v1.5]** / **[v1.6]** / **[v1.7]** / **[v1.8]** / **[v1.9]** / **[v1.10]** / **[v1.11]** / **[v1.12]** / **[v1.13]**.
...
| 1.11 | **Файлы в переписке кабинета** (§9): `feed[].files` — объекты `{ id, name, kind, size }` вместо имён; файл отдаёт `GET /api/account/leads/<K-0042>/files/<id>`. `account.requests.files` — подпись списка файлов («Файлы»), без `{names}` |
| 1.13 | **Файлы в сообщении из кабинета, закрытые заявки** (§9): `POST /api/account/upload/leads/<K-0042>/messages` — multipart `text` + до пяти `files` (фото, видео MP4, PDF, DOCX, TXT; вместе до 50 МБ); ошибка файла называет его (`{ error, file }`). Выполненная и отклонённая заявка — `can_write: false`, сообщения в неё — `409 closed` (вопрос по ней — новым обращением). В `site.json`: `account.requests.{attach, attach_hint, remove}`, `account.errors.{closed, file_type, file_too_big, files_too_big, too_many_files, files_disabled}` (`{file}` — имя файла). `loyalty.priority` в `content/site.yaml` — для админки, в `site.json` его нет |
| 1.12 | **Заявки в кабинете — как мессенджер** (§9): список в одну строку с поиском, открытая заявка — переписка пузырями (описание заявки — первое сообщение клиента), поле ввода прибито к низу, факты заявки — в `<details>` «Детали». В `site.json`: `account.requests.{search, nothing, found, number, time, details, kind, date, message, keys}`, `account.statuses_short.*` — статус в строке списка («ваш ход»); `account.requests.answer` убран — ответы подписаны `profile.nickname` |
...
| `GET /api/account/leads/<K-0042>` | заявка с перепиской: `description`, `budget?`, `timeline?`, `contact`, `method`, `can_write` (**[v1.13]** `false` у выполненной и отклонённой: вместо поля — «Заявка закрыта» и вопрос по ней), `feed: [{ at, kind: message\|status, direction?, channel?, body?, status?, files?: [{ id, name, kind, size }] }]`; помечает прочитанной. `kind` файла — `jpg\|png\|mp4\|pdf\|docx\|txt`, определён по содержимому |
| `GET /api/account/leads/<K-0042>/files/<id>` | **[v1.11]** файл переписки — только владельцу заявки (чужой, спам, обезличенная — `404`). Фото и видео ответа — `inline` с точным типом (для `<img>`, `<video>`), остальное и всё, что прислал сам клиент, — `attachment`, `application/octet-stream`. Всегда `nosniff` и `CSP: sandbox`; запрос только со страниц сайта (`Sec-Fetch-Site: same-origin`) |
| `POST /api/account/leads/<K-0042>/messages` `{ text }` | ответ в заявке; **[v1.13]** в закрытую — `409 closed` |
| **[v1.13]** `POST /api/account/upload/leads/<K-0042>/messages` — `multipart/form-data`: `text` (можно пустой) и до пяти `files` | ответ с файлами: фото (JPG, PNG — до 10 МБ), видео MP4 (до 50 МБ), PDF, DOCX, TXT (до 20 МБ), вместе до 50 МБ; тип — по содержимому. Файл, который не прошёл, останавливает всё сообщение: `422 { error: "file_type" \| "file_too_big", file: "<имя>" }`; ещё `too_many_files`, `files_too_big` (`413`), `files_disabled`, `closed` (`409`). Свой путь — nginx пропускает туда тела до 55 МБ; источник, сессия и `X-CSRF-Token` проверяются до чтения тела |
...
**Разметка кабинета, на которую опирается скрипт** (`web/src/scripts/account.ts`): корень `data-account` с текстами в `data-texts`; виды `data-view="loading|login|dashboard"`; вход — `data-login-methods`, `data-login-email`, `data-login-telegram` (`data-bot-link`), `data-login-code` (`data-sent`, `data-login-back`), `data-login-link` (`data-link-account`, `data-link-yes`, `data-link-no`), `data-login-error`; кабинет — `data-hello`, `data-since`, `data-logout`, `data-loyalty` (`data-next-percent`, `data-next-reason`, `data-tier`, `data-orders`, `data-spent`, `data-progress*`, `data-top`, `data-personal`), `data-screen="requests"` (**[v1.12]** `data-requests-list`, `data-requests-search`, `data-requests-foot`; переписка `data-thread` — `data-thread-title`, `data-thread-status`, `data-thread-details` с `data-thread-facts`, `data-thread-scroll` с `data-thread-feed`, `data-thread-reply`, `data-thread-closed`), `data-inquiry`, `data-achievements-card` (`data-orders` со строками `data-order="<id>"`, `data-eggs-card`), `data-contacts`, `data-profile`, `data-access`, `data-sessions`, `data-delete`; строки списков — `<template id="tpl-request|tpl-row|tpl-msg|tpl-contact|tpl-session">`. Экран заявок занимает окно целиком (`[data-fit]` на корне), прокручиваются только список и переписка.
