 ...
 * matrices, and the sounds are synthesised here after Steam's: the desktop «ding» for an egg, the Steam Deck
 * achievement sound for the last one. Nothing of Valve's is used — no logo, no image, no sound file.
...
function icon(id, rare) {
  const size = 40, dpr = Math.min(devicePixelRatio || 1, 2), n = 9, cell = size / (n + 1);
  const c = document.createElement('canvas');
  c.width = c.height = size * dpr;
  css(c, `display:block;position:relative;width:${size}px;height:${size}px` + (rare ? `;box-shadow:0 0 2px 1px rgba(255,184,78,.6),0 0 16px 1px rgba(255,184,78,.4)` : ''));
  const g = c.getContext('2d');
  if (!g) return c;
  g.scale(dpr, dpr);
  const bg = g.createLinearGradient(0, 0, 0, size);
  bg.addColorStop(0, rare ? '#3b2f19' : '#2a3040');
  bg.addColorStop(1, rare ? '#140f08' : '#11151c');
  g.fillStyle = bg;
  g.fillRect(0, 0, size, size);
  const lit = rare ? GOLD : '#9d86f0';
  (ICONS[id] || ICONS.all).forEach((row, y) => [...row].forEach((ch, x) => {
    g.globalAlpha = ch === '#' ? 1 : ch === '.' ? 0.4 : 0.08;
...
function host() {
  ...
  if (!box) {
    box = el('div', `position:fixed;right:0;width:${WIDTH}px;height:60px;display:flex;align-items:center;z-index:60;pointer-events:none`, document.body);
...
function toast(a) {
  const t = el('div', `display:flex;flex-direction:row;align-items:center;width:${WIDTH}px;min-height:40px;max-height:60px;background-color:#0e141b;`
...
/** Shows an achievement: { id, name, text, count, total, rare, found } — found is read out before the name. */
...
const NOTE = { F2: 87.31, F3: 174.61, C4: 261.63, F4: 349.23, Bb4: 466.16, F5: 698.46, A5: 880, Bb5: 932.33, C6: 1046.5, G6: 1567.98 };
...
/** Plays «egg» or «rare» — into the page's audio, or at «when» into the given context (an OfflineAudioContext in tests). */
...
function play(kind, c, t) {
  try {
    // About as loud and as long as Steam's own: 0.6 s for an egg, 1.6 s for the last one.
    const [level, end] = kind === 'rare' ? [0.19, 1.6] : [0.09, 0.62];
    ...
    wet.gain.value = kind === 'rare' ? 0.16 : 0.4;
    const tail = room(c, kind === 'rare' ? 1.4 : 0.7, kind === 'rare' ? 0.3 : 0.18);
